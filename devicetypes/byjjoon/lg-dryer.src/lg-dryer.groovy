 ...
 *  LG Dryer
 *
 * Original : https://raw.githubusercontent.com/fison67/LG-Connector/master/devicetypes/fison67/lg-dryer.src/lg-dryer.groovy
*/
...
metadata {
	definition (name: "LG Dryer", namespace: "fison67", author: "fison67", ocfDeviceType:"oic.d.dryer") {
        capability "Sensor"
        capability "Switch"
...
def setStatus(data){
    ...
    if(jsonObj.State != null){
        ...
        sendEvent(name:"curState", value: STATE_VALUE[jsonObj.State.value as int]["str"]["KR"])
    }
    
    if(jsonObj.ProcessState != null){
    	def value = PROCESS_STATE_VALUE[jsonObj.ProcessState.value as int]["str"]["KR"]
        if(jsonObj.State.value as int == 0){
        	value = "OFF"
        }
    ...
    if(jsonObj.Course != null){
        ...
        if(item){
        	name = COURSE_VALUE[jsonObj.Course.value as int]["str"]["KR"]
        }
